...
//
// CLI seed pipeline for the Task 2 AI architecture: synthetic event corpus
// → embeddings → cosine clustering → LLM summaries. Writes to log_events,
// log_clusters, log_cluster_members, cluster_summaries.
//
// Run:
//   npm run seed
//
...
//   OPENROUTER_API_KEY
//   AI_BACKEND=openrouter         (or 'ollama' for local)
//   OPENROUTER_MODEL              (optional; defaults to openai/gpt-4o-mini)
//   OPENROUTER_EMBEDDING_MODEL    (optional; defaults to openai/text-embedding-3-small)
//
// DESTRUCTIVE: deletes existing rows in log_events / log_clusters /
// log_cluster_members / cluster_summaries before inserting. Safe to re-run.
//
// Runtime expectations (measured vs inferred shown explicitly below):
//   - ✓ Validated:  ~2 seconds per OpenRouter LLM summary call (empirical)
//   - ✓ Validated:  ~107 clusters from 191 events at threshold 0.65 (empirical
//                   from earlier benchmark run on the same corpus shape)
//   - ✓ Validated:  ~3.5 minutes total runtime end-to-end locally (corpus
//                   insert + embeddings batch + per-cluster summary loop)
//   - ⚠ Inferred:   ~25-40 clusters at threshold 0.70 (would reduce LLM call
//                   count; not yet validated on team's project)
// No timeout constraints since this runs locally rather than as a serverless
// function — the same workload hits Supabase Edge's 150s wall, this CLI path
// bypasses that wall entirely.
...
import type { NormalizedLogEvent, LogSeverity, LogSourceType } from '../src/types/database'
...
function preflight(): void {
  ...
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY)
    errors.push('SUPABASE_SERVICE_ROLE_KEY missing (needed to bypass RLS)')
  const aiKey = process.env.OPENROUTER_API_KEY ?? ''
  if (activeProvider === 'openrouter') {
    if (!aiKey) errors.push('OPENROUTER_API_KEY missing')
    else if (aiKey.startsWith('REPLACE') || aiKey.startsWith('your-'))
      errors.push(`OPENROUTER_API_KEY looks like a placeholder ("${aiKey.slice(0, 10)}…")`)
  }
  if (errors.length > 0) {
    console.error('[seed] pre-flight failed:')
...
// ---------------------------------------------------------------------------
// Timestamp helpers — incidents spread over 7 days with intra-incident jitter.
// Deterministic: jitter uses index arithmetic, not Math.random().
// ---------------------------------------------------------------------------
...
// Start of each incident (days ago)
const INC = {
  disk:    NOW - 6 * DAY,          // day 1: disk/RAID/SMART cascade
  auth:    NOW - 5 * DAY,          // day 2: brute force / credential stuffing
  memory:  NOW - 4 * DAY,          // day 3: memory pressure → OOM → DB cascade
  db:      NOW - 4 * DAY + 3 * HOUR, // day 3 (afternoon): DB specific events
  network: NOW - 3 * DAY,          // day 4: link flapping / STP
  ups:     NOW - 3 * DAY + 4 * HOUR, // day 4 (evening): UPS / power event
  tls:     NOW - 2 * DAY,          // day 5: TLS cert expiry chain
  backup:  NOW - 1 * DAY,          // day 6: backup failures
  service: NOW - 18 * HOUR,        // day 6 (evening): service crashes
  kernel:  NOW - 5 * DAY + 6 * HOUR, // day 2 (morning): kernel/hardware errors
  ntp:     NOW - 2 * DAY + 6 * HOUR, // day 5: NTP drift
  thermal: NOW - 1 * DAY + 8 * HOUR, // day 6 (daytime): thermal/fan alerts
  dns:     NOW - 1 * DAY + 12 * HOUR, // day 6 (noon): DNS failures
  routine: NOW - 7 * DAY,          // scattered routine events
}

// Spread events within an incident: offset in minutes from incident start,
// with a small deterministic jitter per event index.
...
type E = Omit<NormalizedLogEvent, 'timestamp'> & { ts: Date }

// ---------------------------------------------------------------------------
// Corpus — 200+ events, 15 incident themes
// ---------------------------------------------------------------------------
// Each theme is annotated with what the AI SHOULD find:
//   - Intra-theme: high similarity (same root cause, different phrasing)
//   - Cross-source: same root cause visible in syslog AND prtg AND windows_event
//   - Cascade: events that cause each other (disk error → EXT4 → app → backup)
//   - False-positive trap: similar-sounding events that are actually distinct
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Cosine similarity + clustering (same logic as seed-pipeline.ts)
// ---------------------------------------------------------------------------
...
function severityDist(events: NormalizedLogEvent[]): Record<LogSeverity, number> {
  const d: Record<LogSeverity, number> = { debug: 0, info: 0, warning: 0, error: 0, critical: 0 }
  for (const e of events) d[e.severity]++
  return d
}

function distinctSourceTypes(events: NormalizedLogEvent[]): LogSourceType[] {
  return Array.from(new Set(events.map((e) => e.source_type)))
}
...
async function main(): Promise<void> {
  ...
  console.log(`[seed] AI provider: ${activeProvider}`)
  console.log(`[seed]   embedding model: ${activeEmbeddingModel}`)
  console.log(`[seed]   text model:      ${activeTextModel}`)
  console.log(`[seed] pipeline version: ${PIPELINE_VERSION}`)
  console.log(`[seed] clustering threshold: ${CLUSTERING_THRESHOLD}`)
  console.log(`[seed] corpus size: ${CORPUS.length} events`)
  console.log('')

  // STEP 0: Truncate — wipe all prior seeded data
  console.log('[seed] truncating existing data…')
  const { error: truncErr } = await supabase.rpc('truncate_seed_tables' as never)
  if (truncErr) {
    // Fallback: delete in dependency order if the RPC doesn't exist
    console.log('[seed]   (no truncate RPC, deleting in order)')
  // Alarms table delete deferred until the alarms migration is applied on remote.
    await supabase.from('cluster_summaries').delete().neq('id', '00000000-0000-0000-0000-000000000000')
    await supabase.from('log_cluster_members').delete().neq('cluster_id', '00000000-0000-0000-0000-000000000000')
    await supabase.from('log_clusters').delete().neq('id', '00000000-0000-0000-0000-000000000000')
    await supabase.from('log_events').delete().neq('id', '00000000-0000-0000-0000-000000000000')
  }
  console.log('[seed]   done.\n')

  // STEP 1: Build events with timestamps
  const events: NormalizedLogEvent[] = CORPUS.map((e) => ({
  ...
  // STEP 2: INSERT log_events (service-role bypasses RLS)
  ...
  const insertEvents = await supabase.from('log_events').insert(eventRows).select('id, message')
  if (insertEvents.error) throw new Error(`log_events INSERT failed: ${insertEvents.error.message}`)
  const inserted = insertEvents.data ?? []
  console.log(`[seed] inserted ${inserted.length} log_events`)

  // STEP 3: Compute embeddings (batch — single API call)
  const messages = inserted.map((r) => r.message)
  const embRes = await generateEmbeddings(messages)
  console.log(`[seed] computed ${embRes.embeddings.length} embeddings (${embRes.provider}/${embRes.model})`)

  // STEP 4: Cluster
  const labels = clusterByThreshold(embRes.embeddings, CLUSTERING_THRESHOLD)
  const clusterCount = Math.max(...labels) + 1
  console.log(`[seed] formed ${clusterCount} clusters at threshold ${CLUSTERING_THRESHOLD}`)
  console.log('')

  // STEP 5: Insert clusters + members + summaries
  ...
  for (let label = 0; label < clusterCount; label++) {
    ...
    const times    = memberEvents.map((e) => e.timestamp.getTime())
    const firstSeen = new Date(Math.min(...times))
    const lastSeen  = new Date(Math.max(...times))
    const sevDist   = severityDist(memberEvents)
    const sourceTypes = distinctSourceTypes(memberEvents)
    ...
    const insertCluster = await supabase
      ...
      .insert({
        ...
        hierarchy_level:          0,
        embedding_model_provider: embRes.provider,
        embedding_model_name:     embRes.model,
    ...
    const clusterId = insertCluster.data.id

    const refEmb = embRes.embeddings[memberIndices[0]]
    const memberRows = memberIndices.map((idx, i) => ({
      cluster_id:       clusterId,
      log_event_id:     memberDbRows[i].id,
      similarity_score: cosine(refEmb, embRes.embeddings[idx]),
    }))
    const insertMembers = await supabase.from('log_cluster_members').insert(memberRows)
...
${memberEvents
  .slice(0, 10)
  .map((e) => `[${e.severity}] ${e.source_id}: ${e.message}`)
...
Time range: ${firstSeen.toISOString()} → ${lastSeen.toISOString()}`
    ...
    totalSummaryChars += summaryRes.text.length

    const insertSummary = await supabase.from('cluster_summaries').insert({
      cluster_id:             clusterId,
      summary_text:           summaryRes.text,
      summary_model_provider: summaryRes.provider,
      summary_model_name:     summaryRes.model,
      pipeline_version:       PIPELINE_VERSION,
    ...
    const preview = summaryRes.text.replace(/\s+/g, ' ').slice(0, 120)
    const multiSource = sourceTypes.length > 1 ? ` [${sourceTypes.join('+')}]` : ''
    console.log(`  cluster ${label.toString().padStart(3)} (${memberEvents.length.toString().padStart(3)} events${multiSource}): "${preview}…"`)
  }

  console.log('')
  console.log('[seed] PIPELINE COMPLETE')
  console.log(`  corpus events:     ${CORPUS.length}`)
  console.log(`  log_events:        ${inserted.length}`)
  console.log(`  log_clusters:      ${clusterCount}`)
  console.log(`  cluster_summaries: ${clusterCount}`)
  console.log(`  singletons:        ${labels.filter((l, i) => labels.filter(x => x === l).length === 1).length}`)
  console.log(`  multi-event:       ${clusterCount - new Set(labels.filter(l => labels.filter(x => x === l).length === 1)).size}`)
  console.log(`  avg cluster size:  ${(inserted.length / clusterCount).toFixed(2)} events`)
  console.log(`  total summary chars: ${totalSummaryChars}`)
}
