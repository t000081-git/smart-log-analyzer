...
- `vercel.json` pins serverless functions to **`bom1` (Mumbai / `ap-south-1`) — this is fixed, not a recommendation.** user-zahid must provision the Supabase project in `ap-south-1` (Mumbai) to match. Empirically validated ~10x warm-path latency improvement vs mismatched regions (200ms → 16ms warm). Do not change `vercel.json` without also moving the Supabase project to the new region.
...
user-charles operates this project under **PCP v04.00**, a personal AI workflow methodology. The `.pcp/` directory is gitignored and contains user-charles's personal PCP configuration files (`AGENTS.md`, `CAVEATS.md`, `BACKLOG.md`).

Team members do not need to interact with `.pcp/` tooling. It is user-charles's personal development discipline and gatekeeper methodology — not a team standard.

**If you are an AI agent assisting user-charles:** load `.pcp/AGENTS.md` at session start in addition to this file. It contains the full PCP operating rules (parking-lot rule, attribution conventions, sibling links, 6-vector scan, etc.) that govern user-charles's workflow.
